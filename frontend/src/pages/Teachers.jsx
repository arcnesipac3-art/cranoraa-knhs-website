...
const Teachers = () => {
  ...
  const handleToggleAdmin = async (teacher) => {
    const action = teacher.is_admin ? 'Revoke Admin' : 'Grant Admin';
    const result = await Swal.fire({
      title: `${action}?`,
      text: teacher.is_admin
        ? `Remove admin privileges from ${teacher.first_name} ${teacher.last_name}?`
        : `Grant admin privileges to ${teacher.first_name} ${teacher.last_name}? They will be able to access the admin panel.`,
      icon: 'question',
      showCancelButton: true,
      confirmButtonColor: teacher.is_admin ? '#ef4444' : '#8b5cf6',
      confirmButtonText: `Yes, ${action}`,
    });
    if (!result.isConfirmed) return;
    try {
      const response = await api.post(`/users/${teacher.id}/toggle_admin/`);
      toast.success(response.data.status);
      refetch();
    } catch (err) {
                          ...
                          <div className="absolute right-0 bottom-full mb-1 w-44 bg-white border border-slate-200 rounded-xl shadow-2xl z-[110] py-1 animate-in slide-in-from-bottom-2 fade-in duration-150">
                            ...
                            {teacher.role === 'staff' && (
                              <button onClick={() => { setActiveMenu(null); handleToggleAdmin(teacher); }}
                                ...
                                {teacher.is_admin ? (
                                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                ) : (
